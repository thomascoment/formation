        ...
        const main = async () => {
            try {
                ...
                if( !response || !response.text ) {
                    console.error("LA réponse de l'API est vide ou n'a pas de propriété 'text'", repsonse);
                    return;
                }
                ...
                console.error("Erreur lors de l'appel à l'API ou du traitement de la réponse :", error);
             }
            }
    ...
    <div class="cards">
        <div class="card-container">
            <div :class="card.classDiv1" v-for="(card, index) in contentCards" :key="index">
                ...
                <div :class="card.classDiv2">
                    <textarea v-model="promptsValues[card.vModel]" @input="updatePrompt(card.vModel, $event.target.value)"
                        :class="card.textareaClass" :type="card.type" :placeholder="card.placeholder"
                        :maxlength="card.maxlength"></textarea>
                </div>
            </div>
        </div>
        <div class="button-footer">
            <input v-model="newCard" style="display: none;">
            <button @click="generateAndSend()" class="generate">Générer</button>
